...
// --- NEW PLAYLIST MENU TOGGLE FUNCTION ---
window.togglePlaylistMenu = function(e) {
    e.stopPropagation();
...
document.addEventListener('click', (e) => {
    ...
    // Closes the new playlist dropdown if you click anywhere else
    if (!e.target.closest('.wmp-playlist-dropdown-container')) {
        const plMenu = document.getElementById('playlist-dropdown-menu');
...
function updateCornerOverlay(isYouTube) {
    ...
    const ytSrc = "https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/tm8zk7I.png";
    const wpSrc = "https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/GYUWqEX.png";
...
function togglePlaylist() {
    ...
    if (centerPane) {
        if (playlistVisible) {
            // Info Center (Right Pane) is VISIBLE: Center pane stays in column 2
            centerPane.style.gridColumn = '2'; 
        } else {
            // Info Center is HIDDEN: Center pane stretches across columns 2 and 3
...
function renderPlaylist() {
    ...
    const plDropdownMenu = document.getElementById('playlist-dropdown-menu');
    
    if (npDropdownMenu) npDropdownMenu.innerHTML = '';
    
    if (plDropdownMenu) {
        plDropdownMenu.innerHTML = '';
        // Inject the fixed "Load URL" option at the top
        const loadUrlItem = document.createElement('div');
        loadUrlItem.className = 'dropdown-item';
        loadUrlItem.innerHTML = `<div class="dropdown-item-text"><i class="ph-fill ph-globe" style="color: #000; margin-right: 8px;"></i> Open URL...</div>`;
        loadUrlItem.onclick = () => { window.promptNetworkStream(); plDropdownMenu.style.display = 'none'; };
        plDropdownMenu.appendChild(loadUrlItem);
        
        const sep = document.createElement('div');
        sep.className = 'dropdown-separator';
        plDropdownMenu.appendChild(sep);
    }
    
    if (playlist.length === 0) {
        if (npDropdownMenu) npDropdownMenu.innerHTML = '<div class="dropdown-item disabled"><div class="dropdown-item-text">Playlist is empty</div></div>';
        if (plDropdownMenu) plDropdownMenu.innerHTML += '<div class="dropdown-item disabled"><div class="dropdown-item-text">Playlist is empty</div></div>';
    }
    ...
    playlist.forEach((track, i) => {
        ...
        if (plDropdownMenu) {
            ...
            if (i === currentTrackIndex) {
                plItem.style.fontWeight = 'bold';
                plItem.style.background = '#e8edf4';
            }
            plItem.innerHTML = `<div class="dropdown-item-text" style="width: 100%; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;"><i class="ph-fill ${i === currentTrackIndex ? 'ph-play-circle' : 'ph-music-notes'}" style="color: #000; margin-right: 8px;"></i> ${track.title ? track.title : track.file.name}</div>`;
            plItem.onclick = () => {
                loadTrack(i);
                plDropdownMenu.style.display = 'none';
            };
            plDropdownMenu.appendChild(plItem);
        }
    });
}
...
function updatePlayIcon(isPlaying) {
    // wmp logic: The transport button shows what action you will take (click to pause),
...
/* --- SKIN CHOOSER ENGINE --- */
window.confirmBmoPopout = function() {
    if (typeof closeDialog === 'function') closeDialog('bmo-warning-dialog');
    window.triggerPopout('skin-bmo');
};

window.applySelectedSkin = function(skinName) {
    const isPopout = new URLSearchParams(window.location.search).get('popout') === 'true';

    // Intercept BMO skin if we are in the main window
    if (skinName === 'skin-bmo' && !isPopout) {
        if (typeof openDialog === 'function') {
            openDialog('bmo-warning-dialog');
        }
        return; 
    }
    ...
    wmpWindow.classList.remove(
        ...
        'skin-hexagon', 'skin-shield', 'skin-blob'
    ); 
    ...
    if (skinName === 'default') {
        logSystem("Skin applied: Windows Media Player 7");
...
/* --- DYNAMIC BOTTOM UI IMAGE SWAP --- */
document.addEventListener('DOMContentLoaded', () => {
    // 1. Grab the image element that makes up the bottom controls
    const controlsImg = document.querySelector('.wmp7-controls-bg');
    
    // 2. Define our image states
    const imagePaused = 'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/PeZMDSj.png';
    const imagePlaying = 'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/xmB6y5O.png';
    const imageHover = 'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/DlL6Fjn.png'; // <-- New hover image

    // 3. Grab your main media player and the play button
    const player = document.getElementById('main-player'); 
    const playBtn = document.getElementById('play-pause-btn');

    // Safety check to make sure both elements exist on the page
    if (controlsImg && player) {
        
        // When media starts playing, swap to the active image
        player.addEventListener('play', () => {
            controlsImg.src = imagePlaying;
        });

        // When media is paused, swap back to the default image (or hover if mouse is still over it)
        player.addEventListener('pause', () => {
            if (playBtn && playBtn.matches(':hover')) {
                controlsImg.src = imageHover;
            } else {
                controlsImg.src = imagePaused;
            }
        });

        // When media finishes, swap back to the default image
        player.addEventListener('ended', () => {
            controlsImg.src = imagePaused;
        });

        // --- NEW: HOVER LOGIC FOR PLAY BUTTON ---
        if (playBtn) {
            // When mouse enters the play button
            playBtn.addEventListener('mouseenter', () => {
                // Only swap to hover state if the current image is the paused state
                if (controlsImg.src === imagePaused || controlsImg.src.includes('PeZMDSj.png')) {
                    controlsImg.src = imageHover;
                }
            });

            // When mouse leaves the play button
            playBtn.addEventListener('mouseleave', () => {
                // Only revert to paused state if the current image is the hover state
                if (controlsImg.src === imageHover || controlsImg.src.includes('DlL6Fjn.png')) {
                    controlsImg.src = imagePaused;
                }
            });
        }
...
/* --- AUTO-HIDE MEDIA HEADER TEXT --- */
document.addEventListener('DOMContentLoaded', () => {
    const videoHeader = document.querySelector('.wmp-video-header');
    const player = document.getElementById('main-player');
    const centerPane = document.querySelector('.wmp-center-pane');
    
    let headerTimeout;
    let lastMouseX = -1;
    let lastMouseY = -1;

    if (videoHeader && player) {
        // Function to instantly show the header and cancel any active timers
        const showHeader = () => {
            videoHeader.classList.remove('fade-out');
            clearTimeout(headerTimeout);
        };

        // Function to start the 6-second fade countdown
        const startFadeTimer = () => {
            showHeader(); 
            
            // Only hide it if the media is actively playing
            if (!player.paused) { 
                headerTimeout = setTimeout(() => {
                    videoHeader.classList.add('fade-out');
                }, 6000); // 6 seconds
            }
        };

        // Player state triggers
        player.addEventListener('play', startFadeTimer);
        player.addEventListener('pause', showHeader);
        player.addEventListener('ended', showHeader);

        if (centerPane) {
            // Wake up text ONLY if the mouse physically moves to new coordinates
            ...
            // Fast-hide if the cursor leaves the video pane entirely
            centerPane.addEventListener('mouseleave', () => {
                if (!player.paused) {
                    clearTimeout(headerTimeout);
                    headerTimeout = setTimeout(() => {
                        videoHeader.classList.add('fade-out');
                    }, 1000); // Speeds up the fade to 1 second when mouse leaves
                }
            });
        }
    }
...
/* --- UI IMAGE PRELOADER --- */
//quietly load all the hover active states into cache
const imagesToPreload = [
    // Now Playing
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/sOaKtLq.png',
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/SVTCxJE.png',
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/ravXUUp.png',
    // Media Guide
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/mQTecUz.png',
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/hXcCFFy.png',
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/PQuFo31.png',
    // Copy from CD
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/F8b1O1N.png',
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/QloklUK.png',
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/fJav8L7.png',
    // Media Library
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/C4zWIDP.png',
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/gHOcneo.png',
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/CFNeIVx.png',
    // Radio Tuner
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/TIt3bJf.png',
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/Y0ACb5y.png',
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/HCJcVhP.png',
    // Copy to CD or Device
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/V48kZMt.png',
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/SuKp25m.png',
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/KSbxC20.png',
    // Skin Chooser
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/l61n49q.png',
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/IfvEgp7.png',
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/wvw2kBV.png',
    // Core UI Overlays & Backgrounds
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/PeZMDSj.png', // Default Transport
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/xmB6y5O.png', // Active Transport
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/Kw5gUaC.png', // Transport Tile
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/Fvu2urk.png', // Top Swoosh
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/nmT6uZd.png', // Top Logo
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/Ah3oCIl.png', // Corner Overlay
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/y0pTCyk.png',  // Corner Hover
    'https://proxy.duckduckgo.com/iu/?u=https://i.imgur.com/DlL6Fjn.png'  // New hover image
];
...
// Fire the preloader as soon as the DOM is ready!
document.addEventListener('DOMContentLoaded', preloadUIImages);
